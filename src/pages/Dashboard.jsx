...
import {
  ClipboardList, TrendingUp, Clock, ArrowRight, Leaf,
...
const riskConfig = {
  'Low Risk':    { color: 'bg-teal-100 text-teal-700 border-teal-200',       icon: CheckCircle,   dot: 'bg-teal-400'   },
  'Medium Risk': { color: 'bg-yellow-100 text-yellow-700 border-yellow-200', icon: Info,          dot: 'bg-yellow-400' },
  'High Risk':   { color: 'bg-red-100 text-red-700 border-red-200',          icon: AlertTriangle, dot: 'bg-red-400'    },
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // FIX: removed unused getProgress() call — the returned ProgressPoint[]
        // has different fields (date, risk_level) than AssessmentDetailResponse,
        // so it cannot be used interchangeably with history. getHistory() is
        // sufficient for the dashboard display.
        const histRes = await studentAPI.getHistory()
        setHistory(histRes.data || [])
      } catch (e) {
        console.error('Dashboard fetch error:', e)
        setFetchError('Failed to load your dashboard. Please refresh.')
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="blob-bg min-h-screen pb-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        ...
        {fetchError && (
          <div className="mb-6 bg-red-50 border border-red-200 text-red-600 text-sm font-body px-4 py-3 rounded-xl">
            {fetchError}
          </div>
        )}
